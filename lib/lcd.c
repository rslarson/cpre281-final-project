...
void lcd_rotatingBanner(char *data) {
   char lcd[LCD_WIDTH] = {32};
   int i = 0;
   int dataIndex = 0;

   while (true) {
       // Shift the elements of the lcd array left one index
       for (i = 1; i <= LCD_WIDTH - 1; i++) {
           lcd[i - 1] = lcd[i];
       }

       //Determine next char
       if (dataIndex < strlen(data)) {
           lcd[LCD_WIDTH - 1] = data[dataIndex];
           dataIndex++;
       }
       else if (dataIndex < strlen(data) + LCD_WIDTH - 1) {
           lcd[LCD_WIDTH - 1] = 32;
           dataIndex++;
       }
       else {
       ...
       // Clear the screen and print the new sub-message
       lcd_clear();
       lcd_home();
